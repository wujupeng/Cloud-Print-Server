...
func (tm *TaskManager) DispatchTask(ctx context.Context, taskID string) error {
	...
	if tm.docStore != nil && t.DocID != "" {
		if reader, err := tm.docStore.Load(t.DocID); err == nil {
			content, err := io.ReadAll(reader)
			_ = reader.Close()
			if err != nil {
				tm.logger.Warn("read doc content failed", zap.String("doc_id", t.DocID), zap.Error(err))
			} else {
				t.Content = content
			}
		} else {
			tm.logger.Warn("load doc content failed", zap.String("doc_id", t.DocID), zap.Error(err))
		}
	}
